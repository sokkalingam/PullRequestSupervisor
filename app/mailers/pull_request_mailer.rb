class PullRequestMailer < ActionMailer::Base
  ...
  def send_email(pull_requests, users, to, teamName)
    ...
    mail(cc:'sosubramanian@paypal.com', to: to, content_type: "text/html", subject: 'Would you like to review some Pull Requests for ' + teamName + '?')
  end
  ...
  def review_pull_request_email(hours, pull_requests, users, to, teamName)
    ...
    if pull_requests.length > 0
      if hours.include?(time_now.hour) && time_now.min == 0
